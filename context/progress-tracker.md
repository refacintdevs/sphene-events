...
- **Phase 1 Unit 2.5 + payment page (pay button stubbed).** Customer dashboard at `/dashboard/bookings` (gated to CUSTOMER via `(customer)/layout.tsx`): lists the signed-in customer's bookings via `getCustomerBookings()`, mirrors the vendor-bookings card/status-chip style, shows a "Pay deposit" link on ACCEPTED bookings, resolves the prior forward-stub 404 from the booking confirmation page. Payment page at `/book/[serviceId]/pay` (param carries the bookingCode — see AD-012): real ACCEPTED-gated screen via `getBookingForPayment()` (returns null on not-found, ownership mismatch, or status !== ACCEPTED → notFound), displays the deposit amount (depositAmountKobo, not total). The "Pay deposit" button is a DELIBERATE STUB — disabled, labelled in code (`// STUB (Unit 2.3): Paystack initialization not yet wired`) and in the UI ("Payment integration coming in the next build step"). Built ahead of full Paystack work to produce Chapter 4 screenshots; the page, gating, and deposit math are real, only the Paystack call is stubbed. Verified: dashboard lists real bookings; pay page shows deposit for an ACCEPTED booking; a PENDING_VENDOR booking code 404s on the pay page (gate confirmed). Paystack initialization, webhook, and PAID transition remain Unit 2.3.
...
| 2026-06-05 | **AD-011: Static bank list pending Paystack /bank resolution (Unit 2.3)** — The Step 4 bank select uses a hardcoded list from nigerian-context.md (`src/lib/constants/nigerian-banks.ts`) rather than calling Paystack `/bank` (to get the canonical list with codes) or `/bank/resolve` (to verify account number → account name). Reason: Paystack integration is scoped to Unit 2.3. The static list is a progressive-enhancement stop-gap: it lets vendors complete onboarding now and the bank name is stored as a plain string in `VendorProfile.bankName`. Unit 2.3 will add the Paystack Subaccount create call on vendor approval, at which point the bank name resolves against the Paystack bank code list. If the stored string doesn't exactly match Paystack's bank name, a one-time migration or admin step will be needed. This is an accepted, documented trade-off. | Paystack integration not yet in scope; blocking onboarding on it would delay the entire vendor flow. |
...
| 2026-06-08 | **AD-012: Payment page reuses the `[serviceId]` dynamic segment to carry the bookingCode** — The pay page lives at `app/(customer)/book/[serviceId]/pay/page.tsx`. A sibling `[bookingCode]` folder at the same path level as the existing `[serviceId]` segment is not allowed by the Next.js router (two differently-named dynamic segments cannot match the same position). Rather than restructure the existing `/book/[serviceId]` and `/book/[serviceId]/review` routes, the pay route reuses `[serviceId]`, and the page renames the param to `bookingCode` locally. The URL `/book/SE-2026-0014/pay` works because the segment captures any string. CONSEQUENCE: the `[serviceId]` param now carries TWO identifier kinds depending on the route — a real service id for the booking-form pages, a bookingCode for the pay page. Each page must look up the correct entity (pay page → `getBookingForPayment`, form pages → `getServiceForBooking`). When wiring Paystack in Unit 2.3, remember the pay page's param is a bookingCode. | Avoids restructuring working routes under deadline; the segment captures either identifier as a string. Documented because a param named `serviceId` holding a booking code is a non-obvious trap for future edits. |
...
- 2026-06-05: Phase 1 Unit 2.1 Slice B complete. Vendor onboarding form end-to-end. New files: src/lib/constants/nigerian-banks.ts (static list; Paystack /bank deferred to Unit 2.3 — see AD-011); src/lib/validators/vendor.ts (Zod v4 schemas: phone + CAC + bank account + per-step + full onboarding schema); src/lib/cloudinary-upload.ts (XHR upload with progress, 401 surfaces preset errors); src/services/vendor-onboarding.ts (slug generation, profile lookup, single-transaction persist); src/app/(vendor)/vendor/onboarding/actions.ts (Server Action: auth via requireRole, vendorId from DB not client, Zod re-validate, redirect on success per BUG-002); src/components/StepIndicator.tsx; _components/ for Step1–4, FileUpload (single+multi), OnboardingForm, UnderReview. Page.tsx is status router: UNSUBMITTED/null → fresh form, PENDING → UnderReview, APPROVED → redirect('/'), REJECTED/INFO_REQUESTED → pre-filled form + admin note banner. loading.tsx replaced with shape-matched skeleton. CAC is optional (number + cert). Bank is static list (nigerian-banks.ts); Paystack /bank/resolve deferred to Unit 2.3. npm run build passes (11 routes, zero errors). Zod v4 API note: { invalid_type_error, required_error } → { error }.
